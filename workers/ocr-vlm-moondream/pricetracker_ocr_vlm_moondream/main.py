"""FastAPI app worker OCR backend Moondream — POST /push (Pub/Sub `ocr-vlm-moondream`).
...
async def lifespan(app: FastAPI):
    ...
    # Poids `.mf` depuis le bucket models (cold start). Un échec ici fait
    # échouer le démarrage : Cloud Run relance, Pub/Sub NACK/retry — un worker
    # sans poids ne doit jamais ACKer de messages.
...
app = FastAPI(
    title="prt-prod-worker-ocr-vlm-moondream",
    docs_url=None,
    redoc_url=None,
    lifespan=lifespan,
)
...
async def push(
    ...
    try:
        ...
        # Même étage EAN que les autres workers.

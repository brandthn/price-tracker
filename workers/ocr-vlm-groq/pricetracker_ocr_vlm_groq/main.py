"""FastAPI app worker OCR backend Groq — POST /push (Pub/Sub `ocr-vlm-groq`).
...
async def lifespan(app: FastAPI):
    ...
    # Le backend est construit une seule fois : un échec ici (clé API absente,
    # mode VLM invalide) fait échouer le démarrage → Cloud Run relance, aucun
    # message n'est ACKé par un worker mal configuré.
...
app = FastAPI(
    title="prt-prod-worker-ocr-vlm-groq",
    docs_url=None,
    redoc_url=None,
    lifespan=lifespan,
)

...
Copié de ``dev_ocr/src/receipt_ocr/backends/vlm/groq_provider.py`` (imports
réécrits vers ``pricetracker_receipt_pipeline``).
"""
...
def resolve_groq_api_key() -> str:
    ...
    raise OcrBackendError(
        ...
        "in the environment (Cloud Run : secret_env depuis prt-prod-groq-api-key)."
    )
...
class GroqProvider(VlmProvider):
    ...
    def _get_client(self) -> object:
        ...
        try:
            ...
            raise ImportError(
                "GroqProvider requires the 'groq' package (déclaré dans le "
                "pyproject.toml de ce worker)."
            ) from exc

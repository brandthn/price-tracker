...
def parse_retry_envelope(body: bytes) -> str:
    """Decode a Pub/Sub push body from the ``ocr-retry`` topic → ``ticket_id``.

    Contrairement à :func:`parse_pubsub_envelope` (notification GCS), le payload
    est notre propre JSON ``{"ticket_id": "..."}`` (publié par le backend).
    """
    try:
        outer = json.loads(body)
    except json.JSONDecodeError as exc:

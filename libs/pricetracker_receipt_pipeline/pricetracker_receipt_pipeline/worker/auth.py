"""Vérif OIDC du JWT Pub/Sub. Calqué sur workers/ocr-llm/pricetracker_ocr_llm/auth.py.

Adapté en factory : la lib ne possède pas de singleton settings — chaque
worker construit sa dépendance FastAPI via ``build_verify_oidc(get_settings)``.
"""
...
def _resolve_audience(settings: BaseWorkerSettings, request: Request) -> str:
...
def build_verify_oidc(
    get_settings: Callable[[], BaseWorkerSettings],
) -> Callable[..., dict]:
    """Construit la dépendance FastAPI ``verify_oidc`` liée au worker appelant."""

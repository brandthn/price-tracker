...
| **5** | **Notre modèle hybride** — 457 M<br>*CLIP + SmolLM2 gelés, projecteur & LoRA écrits par nous* | Architecture complète, **jamais évaluée** — plantage au banc d'essai |
...
> entraîner nos propres modèles. Le hybride (5) est une dette assumée : complet, testé, déployable,
> mais jamais mesuré.
...
| | paddle | ppocrv4 | **groq** *(étalon)* | **le nôtre** *(from scratch)* |
|---|---|---|---|---|
| **Lecture** (1−CER) | 0,111 | 0,074 | **0,790** | **0,113** |
| **Sortie valide** | 0,833 | 0,722 | **1,000** | **1,000** |
| **Rappel produits** | 0,106 | 0,071 | **0,682** | **0,000** |
| **ANLS** | 0,186 | 0,147 | **0,986** | 0,166 |

*18 photos réelles, mêmes métriques pour tous. Le modèle hybride n'a pas de colonne : plantage au banc d'essai.*

**① L'écart avec l'étalon est net.** Groq reste très au-dessus → nous le gardons en production
**en attendant**.

**② Mais notre modèle égale PaddleOCR en lecture** (0,113 vs 0,111) et il est le **seul moteur local
à 100 % de sorties exploitables**. **8,7 M paramètres, entraînés de zéro, sur un GPU gratuit**, face à
des moteurs industriels.

**③ Il lit — il n'extrait pas encore.** 0,000 en rappel produits : il reconnaît des caractères, pas
encore des champs. C'est **la prochaine marche**, pas un échec.

**La découverte qui a débloqué le projet — il hallucinait :**

```
Avant :  'TRADERJOE'S'  →  'Eurospin'        (invente depuis sa mémoire synthétique)
Après :  'TRADERJOE'S'  →  'WAL[UNK]MART'    (il lit vraiment)
```

→ Entraîné sur du synthétique seul, il **récitait sans regarder les pixels**.
...
> pourquoi. La courbe **montait encore** quand nous avons atteint le plafond d'epochs — le levier

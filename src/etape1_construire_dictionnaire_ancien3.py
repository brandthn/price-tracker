...
def trouver_ligne_produit(lignes: list[str], nom_produit: str, marque: str) -> str | None:
    ...
    for ligne in lignes:
        ...
        # ── Ignorer les lignes qui sont des en-têtes de magasin ───────────────
        # (même si l'OCR les déforme, elles contiennent souvent le nom de l'enseigne)
        if any(e in ligne_norm for e in ENSEIGNES_CONNUES):
            continue

        # ── Ignorer les lignes trop courtes pour être un libellé produit ──────
        if len(ligne.strip()) < 5:
            continue

        # Compter combien de mots-clés sont présents dans cette ligne
        score = sum(1 for mc in mots_cles if mc in ligne_norm)

        # On exige AU MOINS 1 mot-clé trouvé ET au moins la moitié du total
        seuil = max(1, len(mots_cles) // 2)
        if score >= seuil and score > meilleur_score:
            meilleur_score  = score
            meilleure_ligne = ligne

    # Sécurité finale : si le meilleur score est 0, on ne retourne rien
    if meilleur_score == 0:
        return None
